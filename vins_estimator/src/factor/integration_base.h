...
class IntegrationBase
{
  public:
    ...
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0, const Eigen::Vector3d &_vel,
                    const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg, const Eigen::Matrix3d &_r_cam)
        : acc_0{_acc_0}, gyr_0{_gyr_0}, linearized_acc{_acc_0}, linearized_gyr{_gyr_0},
          linearized_ba{_linearized_ba}, linearized_bg{_linearized_bg},
            jacobian{Eigen::Matrix<double, 15, 15>::Identity()}, covariance{Eigen::Matrix<double, 15, 15>::Zero()},
    ...
    void push_back_imu_wh_odom(double dt, const Eigen::Vector3d &acc, const Eigen::Vector3d &gyr, const Eigen::Vector3d &vel)
    {
        dt_buf.push_back(dt);
        acc_buf.push_back(acc);
        gyr_buf.push_back(gyr);
        vel_buf.push_back(vel);
        propagateIMUWhOdom(dt, acc, gyr, vel);
    }
    ...
    void repropagateIMUWhodom(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
    {
        sum_dt = 0.0;
        acc_0 = linearized_acc;
        gyr_0 = linearized_gyr;
        ...
        covariance.setZero();
        for (int i = 0; i < static_cast<int>(dt_buf.size()); i++)
            propagateIMUWhOdom(dt_buf[i], acc_buf[i], gyr_buf[i], vel_buf[i]);
    }
    ...
    void midPointIntegrationIMUWhodom(double _dt,
                            const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                            const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1,
                            const Eigen::Vector3d &delta_p, const Eigen::Quaterniond &delta_q, const Eigen::Vector3d &delta_v,
                            const Eigen::Vector3d &linearized_ba, const Eigen::Vector3d &linearized_bg,
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
        Eigen::Quaterniond q_trans(result_delta_q.toRotationMatrix() * r_cam.transpose());
        Vector3d un_vel = result_delta_q * delta_v;


        //std::cout << "delta p:" << delta_p << std::endl;
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt;
        result_delta_v = delta_v + un_acc * _dt;
        //result_delta_p = delta_p + un_vel * _dt + 0.5 * un_acc * _dt * _dt;
        //result_delta_v = un_vel + un_acc * _dt;

        //        std::cout << "delta p:" << delta_p << std::endl;
        //        std::cout << "delta v:" << delta_v << std::endl;
        //        std::cout << "result_delta_p:" << result_delta_p << std::endl;
        //        std::cout << "result_delta_v:" << result_delta_v << std::endl;

        //std::cout << "trans Q: " << q_trans.x() << "," << q_trans.y() << "," << q_trans.z() << "," << q_trans.w() << std::endl;
        //std::cout << "delta_q: " << delta_q.x() << "," << delta_q.y() << "," << delta_q.z() << "," << delta_q.w() << std::endl;
    ...
    void propagateIMUWhOdom(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1, const Eigen::Vector3d &_vel_1)
    {
        dt      = _dt;
        acc_1   = _acc_1;
        gyr_1   = _gyr_1;
        ...
        midPointIntegrationIMUWhodom(_dt, acc_0, gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v,
                                      linearized_ba, linearized_bg,
                                      result_delta_p, result_delta_q, result_delta_v,
                                      result_linearized_ba, result_linearized_bg, 1);
